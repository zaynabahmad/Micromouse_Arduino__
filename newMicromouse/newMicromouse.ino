...
// Pin configuration
const int leftEncoderPinA = 7; // Phase A pin for encoder 1
const int leftEncoderPinB = 8; // Phase B pin for encoder 1 
const int rightEncoderPinA = 2; // Phase B pin for encoder 1 
...
volatile int lastEncoderBState=0;
volatile int lastEncoderAState =0;
...
void setup()
{
    ...
    attachInterrupt(digitalPinToInterrupt(leftEncoderPinB),leftEncoderHandler,CHANGE);
    attachInterrupt(digitalPinToInterrupt(rightEncoderPinA),rightEncoderHandler,CHANGE);
    attachInterrupt(digitalPinToInterrupt(rightEncoderPinB),rightEncoderHandler,CHANGE);
...
// the ISR  for the encoder
 void leftEncoderHandler() {
  int newEncoderAState = digitalRead(leftEncoderPinA);
  int newEncoderBState = digitalRead(leftEncoderPinB);
 ...
 void rightEncoderHandler() {
  int newEncoderAState = digitalRead(rightEncoderPinA);
  int newEncoderBState = digitalRead(rightEncoderPinB);

  if (newEncoderAState != lastEncoderBState) {
    if (newEncoderBState != newEncoderAState) {
      rightEncoderCount++;
    } else {
      rightEncoderCount--;
  ...
  lastEncoderBState = newEncoderAState;
}
